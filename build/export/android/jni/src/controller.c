...
#define STORY_PATH "assets/script/scene01.txt"

static int menu_item_count(const Controller* c) {
    return c->screen == CONTROLLER_SETTINGS ? 2 : 4;
}
...
static void choose_selected_menu_item(Controller* c) {
    if (c->screen == CONTROLLER_SETTINGS) {
        choose_menu_action(c, c->menu_index == 0 ? VIEW_MENU_AUDIO : VIEW_MENU_BACK);
        return;
    }
    ...
    switch (c->menu_index) {
        ...
        case 3: choose_menu_action(c, VIEW_MENU_QUIT); break;
        default: break;
    }
}
