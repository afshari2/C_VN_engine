...
static SDL_Texture* load_png(SDL_Renderer* r, const char* path) {
    ...
    if (!t) fprintf(stderr, "[View] Cannot load PNG: %s\n", path);
    return t;
}
...
static SDL_Texture* load_texture(SDL_Renderer* r, const char* path) {
    ...
    t = load_bmp(r, path);
    if (!t) fprintf(stderr, "[View] Cannot load texture: %s\n", path);
    return t;
}
...
int view_init(View* v, const char* font_path, int font_size) {
    ...
    v->tex_windowbg = load_bmp(v->renderer, "assets/windowbg.avif");
    v->tex_box      = load_bmp(v->renderer, "assets/textbox.bmp");
    v->tex_heart    = load_png(v->renderer, "assets/heart.png");
    v->tex_bg       = NULL;
    v->tex_sprite   = NULL;
...
static void apply_sprite(View* v, const char* sprite_path, const char* sprite_pos) {
    if (strcmp(v->current_sprite, sprite_path ? sprite_path : "") == 0 &&
        strcmp(v->sprite_pos, sprite_pos ? sprite_pos : "center") == 0) {
        return;
    }
...
void view_set_visuals(View* v, const char* bg_path, const char* sprite_path,
    ...
    if (!v->tex_bg) {
        ...
        apply_sprite(v, safe_sprite, safe_pos);
        return;
    }
...
ViewMenuAction view_menu_action_at(const View* v, int x, int y) {
    int count = v->screen == VIEW_SCREEN_SETTINGS ? 2 : 4;
    int box_x = 190;
    int box_y = v->screen == VIEW_SCREEN_SETTINGS ? 190 : 176;
    int box_w = 260;
    int row_h = 34;
    ...
    if (v->screen == VIEW_SCREEN_SETTINGS) {
        return row == 0 ? VIEW_MENU_AUDIO : VIEW_MENU_BACK;
    }
...
void view_update(View* v) {
    ...
    if (v->blind_state == BLIND_ODD_CLOSE) {
        ...
        if (v->blind_h >= bar_h) {
            ...
            v->tex_bg = load_texture(v->renderer, v->current_bg);
            apply_sprite(v, v->next_sprite, v->next_sprite_pos);
        ...
        if (v->blind_h <= 0) {
            ...
            // Now start the typewriter for the queued text
            v->last_char_time = SDL_GetTicks();
        }
    }
}
...
static void render_choice_box(View* v) {
    if (!v->choices_visible || v->choice_count <= 0 || v->blind_state != BLIND_NONE)
        return;
    ...
    SDL_SetRenderDrawColor(v->renderer, 244, 232, 180, 255);
    SDL_RenderFillRect(v->renderer, &outer);
    SDL_SetRenderDrawColor(v->renderer, 50, 32, 80, 235);
    SDL_RenderFillRect(v->renderer, &inner);
    SDL_SetRenderDrawColor(v->renderer, 118, 77, 122, 255);
    SDL_RenderDrawRect(v->renderer, &outer);

    SDL_Color normal = {246, 238, 210, 255};
    SDL_Color selected = {255, 245, 132, 255};

    for (int i = 0; i < v->choice_count; i++) {
        int y = inner.y + 9 + i * row_h;
        SDL_Color color = i == v->choice_selected ? selected : normal;

        if (i == v->choice_selected) {
            SDL_Rect marker = {inner.x + 14, y + 5, 8, 8};
            SDL_SetRenderDrawColor(v->renderer, 255, 245, 132, 255);
...
static void render_command_button(View* v, int x, int y, const char* label) {
    ...
    SDL_Color text = {255, 236, 148, 255};

    SDL_SetRenderDrawBlendMode(v->renderer, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(v->renderer, 244, 232, 180, 255);
    SDL_RenderFillRect(v->renderer, &outer);
    SDL_SetRenderDrawColor(v->renderer, 45, 29, 77, 242);
    SDL_RenderFillRect(v->renderer, &inner);
    SDL_SetRenderDrawColor(v->renderer, 118, 77, 122, 255);
    SDL_RenderDrawRect(v->renderer, &outer);
...
static void render_command_strip(View* v) {
    ...
    if (v->status_text[0] && SDL_GetTicks() < v->status_until) {
        SDL_Color status = {255, 236, 148, 255};
        render_text_at(v, v->status_text, 374, y + 1, status);
    }
}
...
static void render_menu_row(View* v, int x, int y, int w, const char* label,
    ...
    SDL_Color text = selected
                   ? (SDL_Color){255, 245, 132, 255}
                   : (SDL_Color){246, 238, 210, 255};

    SDL_SetRenderDrawBlendMode(v->renderer, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(v->renderer, 244, 232, 180, 255);
    SDL_RenderFillRect(v->renderer, &outer);
    SDL_SetRenderDrawColor(v->renderer, selected ? 66 : 45,
                           selected ? 38 : 29,
                           selected ? 96 : 77, 244);
    SDL_RenderFillRect(v->renderer, &inner);
    SDL_SetRenderDrawColor(v->renderer, 118, 77, 122, 255);
...
static void render_menu_screen(View* v) {
    ...
    SDL_SetRenderDrawColor(v->renderer, 30, 24, 58, 255);
    SDL_RenderFillRect(v->renderer, &scene);
    SDL_SetRenderDrawColor(v->renderer, 244, 232, 180, 255);
    SDL_RenderDrawRect(v->renderer, &scene);

    SDL_Color title = {255, 245, 132, 255};
    SDL_Color small = {246, 238, 210, 255};
    render_text_at(v, "A CAT IN GIRLS DORMITORY", 148, 82, title);
    render_text_at(v, "C VISUAL NOVEL ENGINE", 205, 112, small);

    int x = 190;
    int y = 176;
    int w = 260;
    const char* labels[] = {"NEW GAME", "LOAD", "SETTINGS", "QUIT"};
    for (int i = 0; i < 4; i++)
        render_menu_row(v, x, y + i * 34, w, labels[i], v->menu_selected == i);

    if (v->status_text[0] && SDL_GetTicks() < v->status_until) {
        SDL_Color status = {255, 236, 148, 255};
        render_text_at(v, v->status_text, 238, 328, status);
    }
...
static void render_settings_screen(View* v) {
    ...
    SDL_Color title = {255, 245, 132, 255};
    SDL_Color small = {246, 238, 210, 255};
    render_text_at(v, "SETTINGS", 276, 110, title);
    render_text_at(v, "AUDIO OUTPUT", 248, 144, small);

    char audio_label[64];
    snprintf(audio_label, sizeof(audio_label), "AUDIO: %s",
             v->audio_enabled ? "ON" : "OFF");

    render_menu_row(v, 190, 190, 260, audio_label, v->menu_selected == 0);
    render_menu_row(v, 190, 224, 260, "BACK", v->menu_selected == 1);

    if (v->status_text[0] && SDL_GetTicks() < v->status_until) {
        SDL_Color status = {255, 236, 148, 255};
        render_text_at(v, v->status_text, 246, 286, status);
    }
...
void view_render(View* v) {
    ...
    if (v->tex_sprite) {
        int tex_w = 0, tex_h = 0;
        SDL_QueryTexture(v->tex_sprite, NULL, NULL, &tex_w, &tex_h);
        if (tex_w > 0 && tex_h > 0) {
    ...
    if (v->typing_done && v->blind_state == BLIND_NONE && v->tex_heart) {
        SDL_Rect heart_rect = {
