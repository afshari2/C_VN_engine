...
static void prepare_var_change_messages(Model* m, const StoryLine* sl,
                                        int delta, int current_value) {
    const char* action = delta >= 0 ? "increased" : "reduced";
    int amount = delta >= 0 ? delta : -delta;

    init_line(&m->message_lines[0], LINE_DIALOGUE, sl->bg_path,
              sl->sprite_path, sl->sprite_pos);
    init_line(&m->message_lines[1], LINE_DIALOGUE, sl->bg_path,
              sl->sprite_path, sl->sprite_pos);

    snprintf(m->message_lines[0].text, sizeof(m->message_lines[0].text),
             "*** %s %s by %d ***", sl->var_name, action, amount);
    snprintf(m->message_lines[1].text, sizeof(m->message_lines[1].text),
             "Current %s %d", sl->var_name, current_value);

    m->message_count = 2;
    m->message_index = 0;
}

static void apply_var_change(Model* m, StoryLine* sl, int delta) {
    int current_value = get_var(m, sl->var_name) + delta;
    set_var(m, sl->var_name, current_value);
    m->current++;
    prepare_var_change_messages(m, sl, delta, current_value);
}

static void skip_control_lines(Model* m) {
    while (m->current < m->line_count) {
        if (m->message_index < m->message_count)
            return;
        ...
        switch (sl->type) {
            ...
            case LINE_ADD:
                apply_var_change(m, sl, sl->value);
                return;
...
int model_load(Model* m, const char* path) {
    ...
    while (story_file_gets(&story_file, buf, sizeof(buf)) && m->line_count < MAX_LINES) {
        ...
        if (strncmp(buf, "REDUCE ", 7) == 0) {
            char var_name[64];
            int value = 0;
            if (sscanf(buf + 7, "%63s %d", var_name, &value) == 2) {
                StoryLine* sl = &m->lines[m->line_count++];
                init_line(sl, LINE_REDUCE, current_bg, current_sprite, current_sprite_pos);
                copy_text(sl->var_name, sizeof(sl->var_name), var_name);
                sl->value = value;
            }
            continue;
...
const StoryLine* model_current_line(const Model* m) {
    if (m->message_index < m->message_count)
        return &m->message_lines[m->message_index];
...
int model_finished(const Model* m) {
    if (m->message_index < m->message_count) return 0;
