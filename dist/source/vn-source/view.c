...
static const SDL_Color UI_PANEL_FILL      = {244, 232, 180, 255}; // Classic warm parchment beige
static const SDL_Color UI_BUTTON_FILL     = {74, 16, 21, 255};   // Deep, dark oxblood red
static const SDL_Color UI_BUTTON_SELECTED = {105, 20, 27, 255};  // Slightly brighter crimson-maroon for selection
static const SDL_Color UI_BORDER          = {140, 36, 43, 255};  // Burnt red-oxide border
static const SDL_Color UI_TEXT            = {253, 245, 230, 255}; // Clean old-lace white for high contrast readable text
static const SDL_Color UI_TEXT_HOT        = {255, 215, 0, 255};   // Bright gold for highlighted/active elements
static const SDL_Color UI_STATUS_TEXT     = {255, 179, 71, 255};  // Warm amber for secondary status readouts
static const SDL_Color UI_FALLBACK_BG     = {43, 12, 15, 255};   // Very dark burgundy background safety blanket
...
int view_init(View* v, const char* font_path, int font_size) {
    ...
    v->tex_menubg   = load_png(v->renderer, "assets/black.avif");
    v->tex_bg       = NULL;
    v->tex_sprite   = NULL;
...
ViewMenuAction view_menu_action_at(const View* v, int x, int y) {
    int count = v->screen == VIEW_SCREEN_SETTINGS ? 3 : 4;
    int box_x = 190;
    int box_y = v->screen == VIEW_SCREEN_SETTINGS ? 176 : 176;
    int box_w = 260;
    int row_h = 34;
...
static void render_command_button(View* v, int x, int y, const char* label) {
    ...
    render_text_at(v, label, x + 8, y + 1, UI_STATUS_TEXT);
    SDL_SetRenderDrawBlendMode(v->renderer, SDL_BLENDMODE_NONE);
}
...
static void render_command_strip(View* v) {
    ...
    if (v->status_text[0] && SDL_GetTicks() < v->status_until) {
        render_text_at(v, v->status_text, 374, y + 1, UI_STATUS_TEXT);
    }
}
...
static void render_menu_row(View* v, int x, int y, int w, const char* label,
    ...
    SDL_Color text = selected ? UI_TEXT_HOT : UI_TEXT;

    SDL_SetRenderDrawBlendMode(v->renderer, SDL_BLENDMODE_BLEND);
    set_draw_color(v->renderer, UI_PANEL_FILL);
    SDL_RenderFillRect(v->renderer, &outer);
    set_draw_color(v->renderer, selected ? UI_BUTTON_SELECTED : UI_BUTTON_FILL);
    SDL_RenderFillRect(v->renderer, &inner);
    set_draw_color(v->renderer, UI_BORDER);
    SDL_RenderDrawRect(v->renderer, &outer);
...
static void render_menu_screen(View* v) {
    ...
    if (v->status_text[0] && SDL_GetTicks() < v->status_until) {
        render_text_at(v, v->status_text, 238, 328, UI_STATUS_TEXT);
    }
